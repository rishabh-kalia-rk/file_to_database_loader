...
def read_csv(file,schemas):
    ...
    #this is the point where memory allocation occur and speed of operation also affect based on chuncksize.
    return df_reader

def to_sql(df,db_conn_uri, ds_name):
    df.to_sql(
        ds_name, db_conn_uri, if_exists='append',index=False, method="multi"
    )
...
def process_dataset(args):
        ...
        try:
            ...
            print(f'Error processing {ds_name} is complete')
...
def process_files(ds_names=None):
    ...
    schemas =json.load(open(f'{src_base_dir}/schemas.json'))
    #this is SQLAlchemy syntax as we are trying to connect it to Postgres database.

    if not ds_names:
        ds_names= schemas.keys()
    processes = len(ds_names) if len(ds_names) <10 else 10 
    pool = multiprocessing.Pool(processes)
    #10 - choose such that the process need to be done should have value greater than this or equal to
    pd_args = []
